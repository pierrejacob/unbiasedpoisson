...
pmmhclassicalvardf %>% group_by(nmcmc, method, r) %>% summarise(mean = mean(value), sd = sd(value), v = var(value))
```


Quick calculation:
```{r}
# var of SV r = 2, t = 20k: 1.365159e-08
# cost: 4 * 20k = 80000



# SUAVE with R = 50: cost 6k, variance 5e-7

# number of SUAVE to get a variance of 1.365159e-08:
6e-7/1.365159e-08
```
...
g <- g + xlab("method") + ylab("estimate") + ylim(0.002, 0.004)
g <- g + geom_hline(yintercept =  mean(results_uavar), col = "red") +  geom_hline(yintercept = c(estim_lo, estim_hi), linetype = "dashed", color = "red")
g
ggsave(filename = "../output/binomialssm.classicavar1e4.pdf", plot = g, width = 8, height = 5)

g <- ggplot(pmmhclassicalvardf %>% filter(nmcmc == 20000) %>% mutate(tss = paste(method, r)),
            aes(x = tss, y = value)) + geom_point() +
  stat_summary(
    geom = "point",
    fun.y = "mean",
...
g <- g + geom_hline(yintercept =  mean(results_uavar), col = "red") +  geom_hline(yintercept = c(estim_lo, estim_hi), linetype = "dashed", color = "red")
g
ggsave(filename = "../output/binomialssm.classicavar2e4.pdf", plot = g, width = 8, height = 5)
```
