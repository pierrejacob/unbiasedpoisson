...
# res <- sample_unbiasedvar_reservoir(HMC, CoupledHMC, rinit,  h = h, k = k, m = m, lag = lag, x_0 = x_0, natoms = 10)
# # names(res)
# # res$elapsedtime
# res$cost
# res$cost_umcmc
...
nrep <- 5e3
natoms_seq <- c(1, 5, 10, 20)
results_ <-  foreach(irep = 1:nrep, .combine = rbind) %dorng% {
  run <- sample_unbiasedvar_reservoir(HMC, CoupledHMC, rinit,  h = h, k = k, m = m, lag = lag, x_0 = x_0, natoms = natoms_seq)
